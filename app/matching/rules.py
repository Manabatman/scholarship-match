def score_scholarship(profile: dict, scholarship: dict) -> float:
    ...
    if profile_region and scholarship_regions:
        if any(
            profile_region.lower() in region.lower() or region.lower() in profile_region.lower()
    ...
    if profile_needs and scholarship_needs:
        matching_needs = set(n.lower() for n in profile_needs) & set(
            n.lower() for n in scholarship_needs
        )
        if matching_needs:
