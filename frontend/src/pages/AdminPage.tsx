    ...
    <section className="py-12">
      <div className="mx-auto max-w-6xl px-4">
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-2xl font-semibold text-slate-900">Admin – Scholarships</h2>
          <Link
            to="/"
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-4 py-2 text-left font-semibold text-slate-700">Provider</th>
            ...
            <tbody>
              {scholarships.map((s) => (
                <tr key={s.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-2 font-medium text-slate-900">{s.title}</td>
                  <td className="px-4 py-2 text-slate-600">{s.provider ?? "—"}</td>
                  <td className="px-4 py-2 text-slate-600">{s.level ?? "—"}</td>
                  <td className="px-4 py-2">
                    <span
                      className={`rounded px-2 py-0.5 text-xs font-medium ${
                        s.is_active !== false ? "bg-green-100 text-green-800" : "bg-slate-100 text-slate-600"
                      }`}
                    >
